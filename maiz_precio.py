...
def fetch_liquidez_table() -> pd.DataFrame:
    ...
    if not tabla:
        raise ValueError("No se encontró la tabla de maiz_precio en la página.")
...
def save_to_csv(data: pd.DataFrame, folder: str, tipo: str = ""):
    ...
    tipo: prefijo para el nombre del archivo (ej: 'CPI', 'maiz')
    """
    import glob
...
save_to_csv(datos, "historial", tipo='maiz_precio')

# Guardar/actualizar archivo acumulado PBI.csv con columna fecha_carga
acumulado_path = os.path.join('historial', 'maiz_precio.csv')
